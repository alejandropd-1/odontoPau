...
- Usar el artículo relacionado como única ficha pública canónica de cada caso clínico: las tarjetas del tratamiento enlazan al artículo y las URL históricas dejan de renderizar contenido duplicado.
...
- `tratamientos-odontologicos`: Tratamientos, profesionales y tarjetas de casos clínicos pasan a Tina; cada tarjeta abre su artículo canónico sin mantener una segunda ficha pública.
- `rehabilitacion-canonica`: La ruta histórica del caso de implantes redirige directamente al artículo canónico y no a una ficha legacy intermedia.
...
- Interfaz: nueva ruta administrativa protegida, panel editorial custom, sistema visual editorial soportado por Tina y Visual Editing sobre Inicio, índice y detalle de Tratamientos, tarjetas de casos clínicos, Artículos e Instrucciones; las URL legacy de casos solo redirigen al artículo canónico y la fuente productiva continúa siendo JSON local versionado.
