...
- [x] 3.2 Ejecutar el preflight y, con autorización aplicable, publicar el retiro mediante el mismo request protegido e idempotente.
- [x] 3.3 Comprobar que la pieza se excluye de ruta canónica, listados, relaciones, sitemap y metadata pública sin borrar su JSON ni duplicar documentos.
- [x] 3.4 Registrar evidencia mínima y confirmar convergencia completa antes de iniciar la republicación.

## 4. Ciclo real de republicación

- [x] 4.1 Volver la pieza a `published`, guardar desde Tina y comprobar su restauración en Preview antes de modificar producción.
- [x] 4.2 Ejecutar el preflight y publicar la republicación desde Tina mediante un request nuevo y único.
- [x] 4.3 Verificar que producción recupera la ruta y todas las superficies canónicas con el mismo documento y sin duplicados.
- [x] 4.4 Registrar evidencia mínima y demostrar la convergencia final de ramas, índice, panel y deploy.
...
- [x] 5.2 Redactar la guía final para usuarios no técnicos: editar, guardar, revisar Preview, publicar, retirar, republicar y reconocer un resultado saludable.
...
- [x] 5.4 Consolidar tiempos y evidencia no sensible de los tres ciclos y actualizar el handoff reusable para OdontoPia sin copiar modelos, rutas ni allowlists específicos.

## 6. Validación y gate humano

- [x] 6.1 Ejecutar `openspec validate validar-operacion-editorial-tina-en-produccion --strict`, `git diff --check` y las pruebas específicas de request, allowlist, retiro y convergencia.
