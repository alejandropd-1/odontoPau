...
const clinicalCaseFields: TinaField[] = [
  {
    ...
    ui: {
      component: InfoBox({
        message: 'Este caso se muestra como una tarjeta dentro del tratamiento. Al abrirlo, la persona irá al artículo completo relacionado, que se edita por separado desde Artículos.',
      }),
    },
  },
  { name: 'id', label: 'ID del caso', type: 'number', required: true },
  optionalString('articleSlug', 'Artículo relacionado', 'Identificador del artículo completo que se abre desde la tarjeta. Si no resuelve un artículo disponible, la tarjeta no muestra un enlace.'),
  requiredString('paciente', 'Referencia pública del paciente'),
  optionalString('fecha', 'Fecha', 'Texto breve que se muestra en la tarjeta del caso.'),
  ...
  { name: 'imagenes', label: 'Imágenes adicionales', type: 'image', list: true, description: 'Se conservan en el registro del tratamiento. El contenido público completo se administra desde el artículo relacionado.' },
