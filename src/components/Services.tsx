...
export default function Services({ data }: { data: any }) {
  return (
    <section 
      className="py-24 bg-surface-background relative overflow-hidden" 
      id="servicios"
      data-sb-field-path="servicesSection"
    >
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-on-surface mb-4 tracking-tight" data-sb-field-path=".title">
            {data?.title || 'Tratamientos de Vanguardia'}
          </h2>
          <p className="text-lg text-on-surface-variant max-w-2xl mx-auto" data-sb-field-path=".description">
            {data?.description || 'Soluciones dentales precisas diseñadas con tecnología de última generación para resultados duraderos y estéticos.'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {tratamientos.map((service, index) => (
            <motion.div
              ...
              data-sb-object-id={service.filePath}
            >
              <div className="absolute inset-0 border-2 border-primary-container opacity-10 rounded-2xl pointer-events-none"></div>
              <div className="w-14 h-14 rounded-full bg-orange-100 flex items-center justify-center mb-6" data-sb-field-path="icon">
                <service.icon className="text-primary-container w-8 h-8" />
              </div>
              <h3 className="text-2xl font-bold text-on-surface mb-3" data-sb-field-path="tituloHero">
                {service.tituloHero}
              </h3>
              <p className="text-on-surface-variant mb-6 leading-relaxed line-clamp-3" data-sb-field-path="descripcionHero">
                {service.descripcionHero}
              </p>
